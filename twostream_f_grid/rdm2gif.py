...
fig, ax = plt.subplots()
cmap = 'viridis'
im = ax.imshow(matrices[0], cmap=cmap)

def animate(i):
    im.set_array(matrices[i])
    ax.set_title(f'nt {i*2}')  # 设置标题为当前的帧数 
    return [im]

ani = animation.FuncAnimation(fig, animate, frames=np.arange(0, nc, 1), interval=50, blit=True)

# 保存为gif
Writer = animation.writers['pillow']
writer = Writer(fps=10, metadata=dict(artist='Me'), bitrate=1800)
ani.save('my_anifull.gif', writer=writer)
