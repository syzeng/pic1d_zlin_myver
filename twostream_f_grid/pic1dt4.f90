    ...
    real :: deltaf=0         !1.0 for delta_f simulation, 0.0 for full_f

    ! particle array: # of species, particles, velocity grids. First specie is electron
    integer,parameter :: nspecie=2,nparticle=10000,nvelocity=128,nbeam=10000
    ...
    integer,parameter :: ntime=1000,ndiag=2,nmode=4
    integer,parameter :: nd=(ntime-1)/ndiag+1
    integer mode(nmode),nt
    data mode/1,2,3,4/             ! mode # for diagnostics
    ...
    xsize=real(ngrid)*deltax    !xsize=L=网格数ngrid乘网格长deltax
    xfreq=2.0*pi/xsize
    ...
    real dx_inv,dv_inv,delj,xsize,x_inv,xpart,xfreq,vtmp,&
        vth_inv,xvdelt(nvelocity,ngrid),xvdelt2(nvelocity,ngrid),xvf0(nvelocity,ngrid), tempcharge(0:ngrid)
    ...
    !add charge to every space grid
    charge=0.0
    do k=1,nspecie
    ...
    !计算电场。k只是角标，xfreq*real(k)才是真实波数
    field_k(1)=0.0
    do k=1,ngrid/2
...
103 format(e15.5)
    ...
    use parameter
    implicit none
    !integer,parameter :: nhistory=22,ndata=9+2*nmode,hismode=23
    integer,parameter :: nhistory=22,ndata=9+2*4,hismode=23
    integer k
    real time(nd)
    ...
    !     'mode2_imag/T','mode3_real/T','mode3_imag/T','mode4_real/T','mode4_imag/T'/
    ...
    open(hismode,file='phi_mode.out',status='replace')
    write(hismode,102)real(phi_mode(:,1,:)**2+phi_mode(:,2,:)**2)
    close(hismode)
