...
! use gfortran 
!* compile on linux sequentially: "gfortran pic1d.f90"        *
!* compile on linux in parallel:  "gfortran -fopenmp pic1d.f90"        *
!*********************************************************************************** 
Module parameter
...
!1: load physical PDF; 0: uniform for [-vmax,vmax] 
  ...
  integer,parameter :: nspecie=2,nparticle=600000,nvelocity=128
...
! output data is delta_f/f_0, f=f_0+delta_f, \int(f_0)dv=1
     ...
     do j=1,ngrid
        write(nfield,102)phi(j),xvdelt(nvelocity/2:nvelocity-1,j)
     enddo
  endif
