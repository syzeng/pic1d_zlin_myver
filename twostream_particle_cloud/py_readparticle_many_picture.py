...
ndiag = 100  # 假设每100个时间步进行一次诊断  
mystep=1 #每隔mystep个点选一个点
  
# 创建一个子文件夹（如果不存在）  
subfolder = 'scatter_twostream_nparticle_{:d}_ntime_{:d}_ndiag_{:d}'.format(nparticle,ntime,ndiag)  
...
for idiag in range(1, ntime // ndiag + 1):  
    ...
    # 计算中间索引在切片中的实际位置，考虑到步长  
    mid_point_slice_idx = (mid_idx - start_idx) // mystep  

    plt.figure(figsize=(12, 8), dpi=100)  
    marker_size = 0.1  
    # 绘制前一半的点（红色）  
    plt.scatter(arx[start_idx:mid_idx:mystep], arv[start_idx:mid_idx:mystep], s=marker_size, color='red')  
    
    # 绘制后一半的点（蓝色）  
    # 注意：由于切片操作不包括结束索引，所以这里使用 mid_idx + 1 作为起始索引  
    plt.scatter(arx[mid_idx + 1:end_idx:mystep], arv[mid_idx + 1:end_idx:mystep], s=marker_size, color='blue')  
