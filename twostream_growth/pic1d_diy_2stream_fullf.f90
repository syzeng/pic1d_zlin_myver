...
!*********************************************************************************** 
    ...
    integer,parameter :: ntime=400,ndiag=2,nmode=2
    ...
    open(field_E_file,file='Ef_energy.txt',status='replace')
    open(totenergy_file,file='total_perturb_energy.txt',status='replace')
    ...
    do k=1,nspecie
     ...
     !add delta function to electrons,k==1
        ...
        ! open(81,file='initial_particle3.txt',status='replace')
        ! write(81,102)x(:,2),v(:,2)
        ! close(81)
    enddo
    102 format(e14.5)
    end Subroutine load
    ...
    integer,parameter :: nfield=11,particlefile=15,partfx=155,partfv=156
